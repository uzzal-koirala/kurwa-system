<?php
// =============================================
// Kurwa System - Delivery Rider OTP Verification
// =============================================
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['verify'])) {
    ...
    if (!empty($otp_input)) {
        // Check OTP in delivery_riders table
        $stmt = $conn->prepare("SELECT * FROM delivery_riders WHERE email = ? AND otp = ?");
        ...
        if ($result->num_rows > 0) {
            ...
            $update = $conn->prepare("UPDATE delivery_riders SET verified = 1, otp = NULL WHERE email = ?");
...
if (isset($_POST['resend'])) {
    ...
    $stmt = $conn->prepare("UPDATE delivery_riders SET otp = ? WHERE email = ?");
    $stmt->bind_param("ss", $new_otp, $email);
    $stmt->execute();

    // Fetch phone and name to send SMS
    $stmt = $conn->prepare("SELECT full_name, phone FROM delivery_riders WHERE email = ?");
    $stmt->bind_param("s", $email);
    $stmt->execute();
    $rider = $stmt->get_result()->fetch_assoc();

    if ($rider && !empty($rider['phone'])) {
        $sms_message = "Dear " . $rider['full_name'] . ", your new rider verification code is: $new_otp.";
        send_sms($rider['phone'], $sms_message);
    }
...
<head>
    ...
    <title>Verify OTP - Rider Portal</title>
    ...
    <style>
        ...
        .otp-input:focus { border-color: #10b981; background: #fff; outline: none; box-shadow: 0 0 0 4px rgba(16,185,129,0.1); }
...
<body class="flex items-center justify-center min-h-screen px-4 bg-gray-50 font-[Poppins]">
    <div class="bg-white shadow-2xl rounded-3xl p-8 w-full max-w-md text-center">
        <div class="mb-8">
            <div class="w-16 h-16 bg-green-50 text-[#10b981] rounded-2xl flex items-center justify-center mx-auto mb-4">
                <i class="ri-shield-check-line text-3xl"></i>
            </div>
            <h2 class="text-2xl font-bold text-gray-800 mb-1">Rider Verification</h2>
            <p class="text-gray-500 text-sm">Verify your phone number to start delivering.</p>
        </div>
        ...
        <div class="flex flex-col gap-3">
            <button type="submit" name="verify" form="otpForm"
                class="w-full btn-delivery text-white font-bold py-4 rounded-xl shadow-lg shadow-green-100 transition-all transform hover:-translate-y-0.5 active:translate-y-0">
                Verify & Start
            </button>
